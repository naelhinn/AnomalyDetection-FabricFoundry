...
### “could_not_open” when extracting frames
- `cv2.VideoCapture()` usually needs a filesystem path:
  - ✅ `/lakehouse/default/Files/bronze/videos/.../video.mp4`
  - ❌ `Files/bronze/videos/.../video.mp4`

### Delta merge / schema mismatch errors
If you see merge errors on re-run:
- Drop the table and re-run, **or**
- Use `.option("overwriteSchema","true")` and keep types consistent (e.g., `t_start_ms` and `t_end_ms` always `double` or always `long`).

### “Reference frame_number is ambiguous”
This happens if you join two dataframes that both have `frame_number` without renaming.
- Rename label frame fields (e.g., `label_frame_number`) before joining to `silver_frame_index`.

---

## Suggested acceptance checks

**1) Silver windows exist**
```sql
SELECT label_prior, COUNT(*) AS n
FROM dbo.silver_event_windows
GROUP BY label_prior;
...
**2) Gold contains both normals and anomalies (same run_id)**
```sql
WITH latest AS (
  SELECT MAX(run_id) AS run_id FROM dbo.gold_conveyor_event_fact_flat
)
SELECT label_prior, COUNT(*) AS n
FROM dbo.gold_conveyor_event_fact_flat
WHERE run_id = (SELECT run_id FROM latest)
GROUP BY label_prior;
```

**3) Evidence JSON populated**
```sql
SELECT event_id, video_id, LENGTH(evidence_frames_json) AS evidence_len
FROM dbo.gold_conveyor_event_fact
LIMIT 20;
```
